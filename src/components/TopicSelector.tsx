...
export function TopicSelector({ bank, selected, onChange }: TopicSelectorProps) {
  ...
  const selectedTopics = new Set(selected);
  const selectedChallengeCount = bank.problems.filter((problem) =>
    selectedTopics.has(getProblemTopic(problem)),
  ).length;
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-500">
              Pick one or more topics. Only challenges from those topics will be included.
            </p>
          </div>
        </div>
        <div className="rounded-xl border border-sky-400/20 bg-sky-400/5 px-3 py-2 text-right">
          <strong className="block text-sm text-sky-200">{selectedChallengeCount} challenges</strong>
          <span className="text-[10px] uppercase tracking-wider text-slate-500">selected</span>
        </div>
      </div>
              ...
              <span className="flex items-center justify-between gap-2">
                ...
                {isSelected ? (
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-wider text-slate-600">
                    {unavailable ? "Soon" : counts[topic.id]}
                  </span>
                )}
              </span>
